...
{
    public partial class NodeGraphEditorWindow : EditorWindow, ISearchWindowProvider
    {
        [SerializeField]
        VisualTreeAsset m_VisualTreeAsset = default;
        NodeGraph m_NodeGraph;
        NodeGraphView m_NodeGraphView;
        ...
        public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context)
        {
            ...
            Type[] types = AppDomain.CurrentDomain.GetAssemblies().SelectMany(
                assembly => assembly.GetTypes()).Where(type => typeof(NodeView).IsAssignableFrom(type) && type.IsClass &&
                !type.IsAbstract && type != typeof(NodeView) && type != typeof(ResultNodeView)).ToArray();
